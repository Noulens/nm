...
void parseSymbols64(t_file *file, uint8_t *map)
{
	...
	for (int i = 0; i < ehdr->e_shnum; ++i)
	{
		...
		if (ft_strncmp(".dynsym", section_name, strlen(".dynsym")) == 0 &&
		...
		if (ft_strncmp(".dynstr", section_name, strlen(".dynstr")) == 0 &&
		...
		if (ft_strncmp(".symtab", section_name, strlen(".symtab")) == 0 &&
		...
		if (ft_strncmp(".strtab", section_name, strlen(".strtab")) == 0 &&
		    sht[i].sh_type == SHT_STRTAB)
		{
			fprintf(stderr, "%s shdr @ 0x%lx\n", section_name, sht[i].sh_offset);
